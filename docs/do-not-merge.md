...
# Important changes and derepcation
In this release, Druid has made substential changes on querying side to make the system more ANSI SQL compatible. This include changes in behavior in NULL handling, boolean value as well as boolean logic. At the same time, Calcite library has been upgrade to the latest version. While we have documented known query behavior changes, please read the upgrade notes section below carefully, test before rolling out to broad production scenarios while closely monitor the query status.
...
## SQL UNNEST and ingestion falttening
...
For more information, see [Dynamic parameters in the upgrade notes](#dynamic-parameters-1).
